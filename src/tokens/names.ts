...
const ACTION_STATE = ['default', 'hover', 'active', 'focus', 'disabled', 'selected'];
const INPUT_STATE = ['default', 'hover', 'focus', 'error', 'disabled', 'checked'];
const SIZE = ['sm', 'md', 'lg'];
const ELEVATION = ['0', '1', '2', '3'];
const SURFACE_STATE = ['default', 'striped'];
const TYPE_ROLE = ['display', 'heading', 'body', 'label', 'caption'];
const ICON_ROLE = ['default', 'secondary', 'accent'];
const WEIGHT = ['thin', 'medium', 'thick'];
const SENTIMENT = ['danger', 'warning', 'success', 'info'];
...
const SEM: Record<string, Record<string, string[][]>> = {
  action: {
    ...
    measure: [['padding-x', 'padding-y', 'radius', 'gap']],
  },
  input: {
    color: [INPUT_STATE, ['bg', 'fg', 'border', 'placeholder-fg']],
    measure: [SIZE, ['padding-x', 'padding-y', 'height', 'radius']],
  },
  surface: {
    color: [ELEVATION, SURFACE_STATE, ['bg', 'border', 'shadow']],
    measure: [['padding', 'radius']],
  },
  type: {
    color: [TYPE_ROLE, ['fg']],
...
// ponytail: full primitive list doesn't exist yet (gap 2), so only the type and segment
// shape are checked. Replace with the frozen list in Phase 1a.
const PRIM_TYPES = ['color', 'space', 'radius', 'type', 'shadow'];
